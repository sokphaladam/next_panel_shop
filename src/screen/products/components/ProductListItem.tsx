  ...
  return (
    <IndexTable.Row id={item?.id + ''} position={item?.id || 0}>
      ...
      <IndexTable.Cell>
        <Badge
          size="small"
          tone={item?.status === 'OUT_OF_STOCK' ? 'critical' : 'enabled'}
          progress={item?.status === 'OUT_OF_STOCK' ? 'incomplete' : 'complete'}
        >
          {item?.status || ''}
        </Badge>
      </IndexTable.Cell>
        ...
        >
          <ActionList
            items={[
              ...
              {
                content: item?.status === 'OUT_OF_STOCK' ? 'Available' : 'Out Of Stock',
                disabled: loading,
                onAction: () =>
                  handleStatus(item?.status === 'OUT_OF_STOCK' ? Status_Product.Available : Status_Product.OutOfStock),
              },
            ]}
          />
        </Popover>
