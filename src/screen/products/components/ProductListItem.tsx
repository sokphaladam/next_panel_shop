...
import { ControllerShowProductOn } from './ControllerShowProductOn';
...
export function ProductListItem({ item, index }: Props) {
  ...
  return (
    <React.Fragment>
      {item?.sku && item.sku.length > 0 && openShow && (
        <ControllerShowProductOn product={item} open={openShow} setOpen={setOpenShow} />
      )}
