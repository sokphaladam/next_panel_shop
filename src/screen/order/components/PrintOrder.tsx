...
export function PrintOrder(props: Props) {
  ...
  return (
    ...
    >
      <Modal.Section flush>
        <div className="relative flex flex-row justify-center overflow-x-hidden">
          <div ref={contentToPrint}>
            <DispalyInvoice {...props} />
          </div>
        </div>
        <div className="relative flex hidden flex-row justify-center overflow-x-hidden">
...
export function DispalyInvoice(props: Props) {
  ...
  const created_date = props.order
    ? props.order?.log?.find((f) => f?.text === "Created")?.date
    : "";
  const last_date = props.order
    ? props.order?.log?.find((f) => f?.text === "Last Updated")?.date
    : "";
  const verify_date = props.order
  ...
  const signature = props.order
    ? props.order?.log?.find((f) => f?.text === "Signature")?.by?.display
    : "";
  const totalAfterDiscount = Number(
    (Number(props.total) - Number(discount)).toFixed(2)
  );
  ...
  for (const x of props.order?.items || []) {
    ...
    if (find >= 0) {
      ...
      if (groups[find].addons) {
        ...
        const v =
          Number((currentAdd[1] || "").split(")")[0]) +
          Number(newAdd[1].split(")")[0]);
  ...
  return (
    <div
      className="noto-sans-khmer p-1 text-center"
    ...
    >
      ...
      <div className="flex flex-row justify-between gap-1">
        <div className="flex flex-col items-start">
          <p className="flex flex-row items-center justify-end">
            ...
            <div className="text-end">
              #{String(props.order?.invoice).padStart(5, "0")}
            </div>
          </p>
          <p className="flex flex-row items-center justify-end">
            <div className="w-[45px] text-left">Order</div>
            <div className="mx-1">:</div>
            <div className="max-w-[100px] overflow-hidden text-nowrap text-end">
              {verify_date}
            </div>
          </p>
          <p className="flex flex-row items-center justify-between">
            ...
            <div className="mx-1">:</div>
            <div className="max-w-[100px] overflow-hidden text-ellipsis text-nowrap text-end">
              {user?.display || ""}
            </div>
          </p>
          <p className="flex flex-row items-center justify-between">
        ...
        <div>
          ...
          <p className="flex flex-row items-center justify-between">
            ...
            <div className="mx-2">:</div>
            <div className="w-[90px] text-right">
              {moment(new Date(created_date + "")).format("LTS")}
            </div>
          </p>
          <p className="flex flex-row items-center justify-between">
            ...
            <div className="w-[90px] text-end">
              {moment(new Date(last_date + "")).format("LTS")}
            </div>
          </p>
          <p className="flex flex-row items-center justify-between">
            ...
            {groups.map((x, i) => {
              ...
              return (
                <tr key={i} className="border-x border-solid border-black">
                  ...
                  <td className="!border-y-0 !text-center">{x?.qty}</td>
                  <td className="!border-y-0 text-left">
                    ${x?.price?.toFixed(2)}
                  </td>
                  <td className="!border-y-0 text-left">
                    {x?.discount
                  ...
                  <td className="!border-y-0 text-right">
                    ${amount.toFixed(2)}
                  </td>
                </tr>
              );
                  ...
                  <div className="flex flex-col justify-between">
                    {discount > 0 && (
                      <>
                        ...
                        <div className="h-8">
                          Discount ({props.order?.discount?.toFixed(2)}%)
                        </div>
                      </>
                    )}
                  ...
                  <div className="flex flex-col justify-between">
                    {discount > 0 && (
                      <>
                        <div className="h-8">
                          {formatKHR(
                            Math.round(
                              Number(exchangeRate) * Number(props.subtotal)
                            )
                          )}
                        </div>
                        <div className="h-8">
                          {formatKHR(
                            Math.round(Number(exchangeRate) * Number(discount))
                          )}
                        </div>
                      </>
                    )}
                    ...
                    <div className="h-8">
                      {formatKHR(
                        Math.round(
                          Number(exchangeRate) *
                            Number(props.order?.customerPaid)
                        )
                      )}
                    </div>
                    <div className="h-8">
      ...
      {!props.kitchen && (
        <div className="text-center">Thank you, see you again!</div>
      )}
    </div>
  );
}
